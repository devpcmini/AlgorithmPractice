...
public class Quiz_2460 {
	...
	public static void main(String[] args) throws IOException {
		System.out.println("각 역에서 내린 사람 수와 탄 사람 수가 빈칸을 사이에 두고 첫째 줄부터 열 번째 줄까지 역 순서대로 한 줄에 하나씩 주어진다. \r\n");
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		int max = 0, hosunMax = 0;
		for(int i=0; i<10; i++) {
			StringTokenizer st = new StringTokenizer(br.readLine());
			hosunMax = hosunMax - Integer.parseInt(st.nextToken()) + Integer.parseInt(st.nextToken());
			if(max < hosunMax) {
				max = hosunMax;
			}
		}
